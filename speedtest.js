...
s.onupdate = function(data) { //callback to update data in UI
  interface("ip").textContent = data.clientIp;
  interface("dlText").textContent = (data.testState == 1 && data.dlStatus == 0) ? "..." : data.dlStatus;
  interface("ulText").textContent = (data.testState == 3 && data.ulStatus == 0) ? "..." : data.ulStatus;
  interface("pingText").textContent = data.pingStatus;
  interface("jitText").textContent = data.jitterStatus;
  var prog = (Number(data.dlProgress) * 2 + Number(data.ulProgress) * 2 + Number(data.pingProgress)) / 5;
  interface("progress").style.width = (100 * prog) + "%";
}
s.onend = function(aborted) { //callback for test ended/aborted
  interface("startStopBtn").className = ""; //show start button again
...
function startStop() { //start/stop button pressed
  if (s.getState() == 3) {
    ...
    s.start();
    interface("startStopBtn").className = "running";
  }
}

//function to (re)initialize UI
function initUI() {
  interface("dlText").textContent = "";
  interface("ulText").textContent = "";
  interface("pingText").textContent = "";
  interface("jitText").textContent = "";
  interface("ip").textContent = "";
}
